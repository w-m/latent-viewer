...
interface Props {
  gridSize?: number;           // default 3 (NxN)
  cellPx?: number;             // pixel dimension of one cell (square)
...
export const LatentGrid: React.FC<Props> = ({
  ...
  const toCell = (x: number, y: number) => {
    cell[0] = Math.max(0, Math.min(gridSize - 1, (y / cellPx) | 0));
    cell[1] = Math.max(0, Math.min(gridSize - 1, (x / cellPx) | 0));
    return cell;
  };
  ...
  useEffect(() => {
    ...
    if (c[0] !== active[0] || c[1] !== active[1]) {
      setActive([c[0], c[1]]);
      onLatentChange(c[0], c[1], indicatorPos.x / stagePx, indicatorPos.y / stagePx);
    }
  }, [indicatorPos, active, cellPx, gridSize, onLatentChange, stagePx]);
  ...
  const handleDragMove = (e: Konva.KonvaEventObject<DragEvent>) => {
    ...
    const x = Math.max(0, Math.min(stagePx, pos.x));
    const y = Math.max(0, Math.min(stagePx, pos.y));
    ...
    >
      ...
      <Layer listening={false}>
        <Rect
          ...
          fillLinearGradientEndPoint={{ x: stagePx, y: 0 }}
        ...
        <Rect
          ...
          opacity={0.5}
          fillLinearGradientStartPoint={{ x: 0, y: 0 }}
          fillLinearGradientEndPoint={{ x: 0, y: stagePx }}
        ...
        <Rect
          ...
          height={stagePx}
          opacity={0.5}
          fillLinearGradientStartPoint={{ x: stagePx, y: 0 }}
          fillLinearGradientEndPoint={{ x: stagePx, y: stagePx }}
      ...
      <Layer listening={false}>
        {Array.from({ length: gridSize + 1 }, (_, i) => (
          <Line
            key={`v${i}`}
            points={[i * cellPx, 0, i * cellPx, stagePx]}
            stroke="#777"
            strokeWidth={1}
          />
        ))}
        {Array.from({ length: gridSize + 1 }, (_, i) => (
          <Line
            key={`h${i}`}
            points={[0, i * cellPx, stagePx, i * cellPx]}
            stroke="#777"
            strokeWidth={1}
          />
        ))}
